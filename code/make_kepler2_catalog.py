...
def calculate_planet_properties(df):
    ...
    df['noise_level'] = simfuncs.kepler_noise_1h(df.kepmag) 
    ...
    starID = 0  # ???
    newDF, starID = simfuncs.make_allplanets_df_vec_extrap_kepler(df, starID)
    ...
    #  !!!! intil I get contamination ratios
    newDF['transit_depth_diluted'] = newDF['transit_depth']
...
def get_ntransits_primary(row, sectorlength=91.3125, nsectors=124):
    ...
    bins = sectorlength * np.arange(0, 1 + 16 +1)
    inds = np.digitize(transitTimes, bins=bins, right=True)
    inds = inds[inds < 1 + 16 + 1]
    return np.count_nonzero(inds * row.loc[[str(x) for x in inds]])
...
def make_output_arr(dfx, nsectors):
    ...
    dfx.loc[(dfx.planetRadius >= 0.8) & (dfx.planetRadius <= 1.2) &
    (dfx.planetPeriod <= 438.3) & (dfx.planetPeriod >= 292.2 ),
     'inZetaEarth'] = True
    dfx.loc[:, 'inSAG13'] = False
    dfx.loc[(dfx.planetRadius >= 0.5) & (dfx.planetRadius <= 1.5) &
    (dfx.planetPeriod <= 860) & (dfx.planetPeriod >= 237 ),
     'inSAG13'] = True
    return dfx
...
if __name__ == '__main__':
    fn = '../data/bryson/dr25_stellar_berger2019_clean_GK.txt'
    ...
    usecols = [0, 2, 11, 14, 21, 26, 30, 35, 36, 37, 41, 43, 45, 70]
    ...
    selected.to_csv('../data/KeplerBryson-EM-{}-{}T.csv.bz2'.format(consts['version'], consts['detect_transits']),
                    compression='bz2')
    selected.to_csv('../data/KeplerBryson-EM-{}-{}T.csv'.format(consts['version'], consts['detect_transits']),
         
                    )
    out_kepler = np.tile(get_quarters(), [selected.shape[0], 1])
    df_out_kepler = pd.DataFrame(out_kepler, columns=[str(
        x) for x in range(1, 1 + out_kepler.shape[1])])
    dfw_kepler = pd.concat([selected.reset_index(drop=True), df_out_kepler], axis=1)
    nsectors = out_kepler.shape[1]
    dfw_kepler = make_output_arr(dfw_kepler, nsectors)

    print('Planets detected in primary + extended mission: {}'.format(dfw_kepler[dfw_kepler.detected].shape[0]))
    print('Planets detected in primary mission SNE: {}'.format(
        dfw_kepler[dfw_kepler.detected_primary].shape[0]))
    dfw_kepler.to_csv('../data/obs_kepler-{}-{}T.csv'.format(consts['version'], consts['detect_transits']),
                   )
    ...
    for i in trange(50):
        ...
        q3 = np.r_[q3, dfw_kepler[dfw_kepler.detected_primary & (dfw_kepler.inZetaEarth)].shape[0]]
        q4 = np.r_[q4, dfw_kepler[(dfw_kepler.detected) & (dfw_kepler.inZetaEarth)].shape[0]]

        q5 = np.r_[q5, dfw_kepler[dfw_kepler.detected_primary & (dfw_kepler.inSAG13)].shape[0]]
        q6 = np.r_[q6, dfw_kepler[(dfw_kepler.detected) & (dfw_kepler.inSAG13)].shape[0]]