...
def get_camera_bouma(df, fieldfile='../data/camera_boresights_SNE.csv'):
    """
...
def make_output_arr(dfx, nsectors):
    ...
    dfx['detected'] = ((dfx.noise_level < dfx.needed_for_detection) &
                     (dfx.Ntransits >= consts['detect_transits']) & (dfx.planetRadius > 0.0)
                       & dfx.has_transits)
    ...
    dfx['detected_primary'] = ((dfx.noise_level < dfx.needed_for_detection_primary) &
                     (dfx.Ntransits_primary >= consts['detect_transits']) & (dfx.planetRadius > 0.0)
                               & dfx.has_transits)
...
if __name__ == '__main__':
    ...
    out_SNSNS = get_camera_bouma(selected.reset_index(drop=True), fieldfile='../data/camera_boresights_SNSNS.csv')
    out_SNNSN = get_camera_bouma(selected.reset_index(drop=True), fieldfile='../data/camera_boresights_SNNSN.csv')
    out_EC3PO = get_camera_bouma(selected.reset_index(drop=True), fieldfile='../data/camera_boresights_EC3PO.csv')

    df_out_SNE = pd.DataFrame(out_SNE, columns=[str(x) for x in range(1, 1+out_SNE.shape[1])])
    df_out_SNSNS = pd.DataFrame(out_SNSNS, columns=[str(x) for x in range(1, 1+out_SNE.shape[1])])
    df_out_SNNSN = pd.DataFrame(out_SNNSN, columns=[str(x) for x in range(1, 1+out_SNE.shape[1])])
    df_out_EC3PO = pd.DataFrame(out_EC3PO, columns=[str(x) for x in range(1, 1+out_SNE.shape[1])])
